...
const MG_PASSED_PAWN: i16 = 25;
const EG_PASSED_PAWN: i16 = 50;
const BROKEN_PAWN_SHELTER: i16 = -50;
const ROOK_OPEN_FILE: i16 = 30;
const ROOK_SEMI_FILE: i16 = 5;
...
impl Evaluation{
  ...
  pub fn ab_pruning(board: &mut Board, initial_alpha: i16, initial_beta: i16, mve: (u8, u8), depth: u32, maximizing_player: bool) -> (i16, (u8, u8), u32) {
    let mut node_count = 1;
    if depth == 0 {
        return (evaluate_board(board) as i16, mve, node_count);
    }
    // let TTval = transposition_table.lookup(zobrist.compute_hash(board));
    // match TTval{
    //   Some(x) => {
    //     if x.depth >= depth {
    //       if x.node_type == node::EXACT {
    //         return (x.score, x.best_move.unwrap(), node_count);
    //       } else if x.node_type == node::LOWERBOUND {
    //         initial_alpha = initial_alpha.max(x.score);
    //       } else if x.node_type == node::UPPERBOUND {
    //         initial_beta = initial_beta.min(x.score);
    //       }
    ...
    if moves.len() == 0 {
        if is_check(board) {
            if maximizing_player {
                // transposition_table.store(zobrist.compute_hash(board), i16::MIN + depth as i16, 0, node::EXACT, mve);
                return (i16::MIN + depth as i16, mve, node_count);
            } else {
                // transposition_table.store(zobrist.compute_hash(board), i16::MIN + depth as i16, 0, node::EXACT, mve);
                return (i16::MAX - depth as i16, mve, node_count);
            }
        } else {
    ...
    if maximizing_player {
        ...
        for i in (0..moves.len()).step_by(2) {
            ...
            let (score, _, child_node_count) = Self::ab_pruning(&mut new_board, alpha, initial_beta, (moves[i], moves[i + 1]), depth - 1, false);
        ...
        for i in (0..moves.len()).step_by(2) {
            ...
            let (score, _, child_node_count) = Self::ab_pruning(&mut new_board, initial_alpha, beta, (moves[i], moves[i + 1]), depth - 1, true);
            node_count += child_node_count;
...
pub fn evaluate_board(board: & mut Board) -> i16 {
  let mut score: i16 = 0;
  let mut egPhase: i16 = 0;
...
pub fn w_passed_pawn_count(wpawns: u64, bpawns: u64)->i16{
  let mut all_front_spans = black_front_span(bpawns);
  all_front_spans |= east_one(all_front_spans) | west_one(all_front_spans);
  (wpawns & !all_front_spans).count_ones() as i16
}
pub fn b_passed_pawn_count(wpawns: u64, bpawns: u64)->i16{
  let mut all_front_spans = white_front_span(wpawns);
  all_front_spans |= east_one(all_front_spans) | west_one(all_front_spans);
  (bpawns & !all_front_spans).count_ones() as i16
}
...
pub fn calculate_material(board: &Board) -> [i16; 2] {
  ...
  let mut mgscore: i16 = 0;
  let mut egscore: i16 = 0;
  let mut game_phase: i16 = 0;
...
pub fn calculate_pawn_structure(board: &Board, egphase: i16) -> i16 {
  let mut score: i16 = 0;
...
pub fn calculate_king_safety(board: &Board) -> i16 {
  let mut score: i16 = 0;
...
pub fn calculate_bishop_pair(board: &Board) -> i16 {
  let mut score: i16 = 0;
  let mut wbishops = board.bishops() & board.white();
  let mut bbishops = board.bishops() & board.black();
...
pub fn rook_on_open_file(board: &Board) -> i16 {
  let mut score: i16 = 0;
  let mut wrooks = board.rooks() & board.white();
  let mut brooks = board.rooks() & board.black();
...
pub fn rook_on_semi_open_file(board: &Board) -> i16 {
  let mut score: i16 = 0;
  let mut wrooks = board.rooks() & board.white();
  let mut brooks = board.rooks() & board.black();
