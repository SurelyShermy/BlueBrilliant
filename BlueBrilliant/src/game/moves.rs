...
pub fn calculate_valid_moves(index: usize, board_state: &BoardState, defender: bool) -> Vec<usize> {
    let piece = board_state.board[index];
    let mut valid_moves = Vec::new();
    if (piece == 0) {
        return valid_moves;
    }
    if (piece_type(piece) == PAWN) {
        valid_moves = pawn_move_generator(board_state, index, defender);
    } else if (piece_type(piece) == KNIGHT) {
        valid_moves = knight_move_generator(board_state, index, defender);
    } else if (piece_type(piece) == BISHOP) {
        valid_moves = bishop_move_generator(board_state, index, defender);
    } else if (piece_type(piece) == ROOK) {
        valid_moves = rook_move_generator(board_state, index, defender);
    } else if (piece_type(piece) == QUEEN) {
        valid_moves = queen_move_generator(board_state, index, defender);
    } else if (piece_type(piece) == KING) {
        valid_moves = king_move_generator(board_state, index, defender);
    }
    valid_moves
}
